...
Valid `Status` values and their target folders:

| Status | Folder | Set by |
|--------|--------|--------|
| `inbox` | inbox/ | FitFoundry (default) |
| `report-requested` | inbox/ | FitFoundry (Generate full report) |
| `shortlisted` | shortlisted/ | LaunchKit |
| `applied` | applied/ | You (manually) |
| `interviewing` | applied/ | You (manually) |
| `offer` | applied/ | You (manually) |
| `declined` | declined/ | You (manually) |
| `rejected` | closed/ | You (manually) |
| `withdrawn` | closed/ | You (manually) |

### Scoring scale

| Score | Meaning | Action |
|-------|---------|--------|
| 9–10 | Dream job — near-perfect alignment | Individual file in inbox/ |
| 7–8 | Strong fit — worth pursuing | Individual file in inbox/ |
| 6 | Partial fit — one significant gap | Individual file in inbox/ |
| 5 | Partial fit — multiple gaps | Master list only; offer Generate full report |
...
Reorganizes your results folder and syncs files to the correct subfolder based on their `Status:` field. Also migrates the legacy flat `results/` layout used before the 2026-04-04 update.

Open a Cowork session and say: **Run FitFoundry Cleanup**

### Confusion Matrix

Reads all reviewed job reports, computes precision and recall for FitFoundry's scoring, surfaces patterns in false positives and false negatives, and suggests targeted changes to your career profile and scoring calibration. Run weekly for best results.

Open a Cowork session and say: **Run Confusion Matrix**

FitFoundry will remind you if the Confusion Matrix hasn't run in over a week.
