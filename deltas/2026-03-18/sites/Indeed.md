...
get_resume()
```
⚠️ **Skip this step.** The `get_resume()` call consistently hangs and does not return in Cowork sessions. Resume context is available directly from the `resumes/` folder in the workspace — read the appropriate `.docx` file instead. Do not attempt `get_resume()` as a connector validation check; proceed directly to searches.
...
**Connector validation:** ⚠️ Skip `get_resume()` — it hangs reliably in Cowork sessions. Instead, validate the connector is active by running the first `search_jobs` call. If that returns results, the connector is working. For resume context, read the relevant `.docx` file from the `resumes/` folder in the workspace.
