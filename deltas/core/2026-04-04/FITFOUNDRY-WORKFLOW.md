...
**Date format:** All dates written to any file during this session must be in YYYY-MM-DD format (e.g., 2026-04-04). Do not write relative dates such as "3 days ago" or "posted yesterday." Convert any relative dates to absolute using today's date before saving.
...
Create the file: OUTPUT_FOLDER/runs/[YYYY-MM-DD]-[JOB_BOARD_NAME].md

Create the `runs/` subdirectory inside OUTPUT_FOLDER if it does not exist.
...
Create the file: OUTPUT_FOLDER/runs/[YYYY-MM-DD]-[RUN_LABEL].md
...
STEP REVIEW — GENERATE FULL REPORTS

After confirming all files are saved, display the complete scored jobs table from the
master list — all jobs that received a score, regardless of whether an individual file
was created. Include a column indicating whether an individual file exists:

| Score | Title | Company | Report? |
|-------|-------|---------|---------|
| [X]   | ...   | ...     | ✅ / — |

Then ask:

"Would you like a 'Generate full report' for any jobs listed above that don't have
individual files? A full report fetches the complete job description, scores it in
detail, and saves it for review. Reply with row numbers from the table above, or
'none' to finish."

Wait for the user's response.

If the user provides row numbers:

For each requested job:
1. Fetch the full job description using the appropriate method for this board
   (MCP connector: use the board's description retrieval method from the sites/ file;
   Puppeteer: navigate to the posting link and read the full description).
2. Score the job fully against PROFILE_PATH using the calibration table in
   QUICK_REFERENCE_PATH. Update the master list table with the score.
3. Create an individual file:
   Filename: OUTPUT_FOLDER/inbox/[Company]_[JobTitle]_[YYYY-MM-DD].md
   Use the Individual File Format below, but set Status to `report-requested`
   instead of `inbox`. This marks the job as a False Negative signal for the
   Confusion Matrix — it scored below the individual-file threshold but you
   flagged it as worth reviewing.
4. Run the ghost job check for each new file.
5. Append "(Full report generated)" next to the job's row in the master list.

Report when complete. Remind the user that these files are in inbox/ and will
appear in the next Confusion Matrix run as False Negative candidates.

If the user replies 'none', close the session normally.
...
Create the `inbox/` subdirectory inside OUTPUT_FOLDER if it does not exist.

All dates in this file must be in YYYY-MM-DD format.
