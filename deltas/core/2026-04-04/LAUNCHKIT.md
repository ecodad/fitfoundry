...
Scan OUTPUT_FOLDER/inbox/ for individual job files. These are markdown files matching
the pattern: [Company]_[JobTitle]_[YYYY-MM-DD].md

Create OUTPUT_FOLDER/inbox/ if it does not exist.

For each file found:
- Read the Score: and Status: fields from the file header (first 10 lines).
- Check whether a corresponding application folder already exists in
  OUTPUT_FOLDER/shortlisted/. An application folder is a subdirectory named
  [Company]_[JobTitle] (without the date). If the folder exists, LaunchKit has
  already been run for that job.

Build two lists:
- UNPROCESSED: job files in inbox/ with no corresponding shortlisted/ folder
- ALREADY_PROCESSED: job files that already have a shortlisted/ folder
...
4a. Create the application folder.
    Path: OUTPUT_FOLDER/shortlisted/[Company]_[JobTitle]/
    Replace spaces with underscores. Remove special characters.
    Create OUTPUT_FOLDER/shortlisted/ if it does not exist.

4b. Move the job report file into the folder.
    Original path: OUTPUT_FOLDER/inbox/[Company]_[JobTitle]_[YYYY-MM-DD].md
    New path:      OUTPUT_FOLDER/shortlisted/[Company]_[JobTitle]/[Company]_[JobTitle]_[YYYY-MM-DD].md

    After moving, update the Status: field in the file header from `inbox` to
    `shortlisted`. The Status: field is in the first few lines of the file in the
    format: Status: inbox

    Status lifecycle reference (for the user's manual updates later):
    - shortlisted  — actively working on application materials (set by LaunchKit)
    - applied      — application submitted; note the date in a comment if desired
    - interviewing — in active interview process
    - offer        — offer received
    - declined     — decided not to pursue
    - rejected     — applied, received rejection
    - withdrawn    — applied then withdrew
...
4d. Generate a tailored resume.
    ...
    Save as: OUTPUT_FOLDER/shortlisted/[Company]_[JobTitle]/Resume_[Company]_[JobTitle].docx
...
4e. Generate a tailored cover letter.
    ...
    Save as: OUTPUT_FOLDER/shortlisted/[Company]_[JobTitle]/CoverLetter_[Company]_[JobTitle].docx
...
When all jobs are complete, report:
  ...
  ### Next Steps
  - Review each folder in results/shortlisted/. Resume and cover letter are first
    drafts — edit before submitting.
  - When you submit an application, update the Status: field in the job report from
    `shortlisted` to `applied`, then run FitFoundry Cleanup to move the folder to
    results/applied/.
  - For deeper editing and refinement, open the .docx files in Claude Chat or
    your preferred editor.
  - Re-run LaunchKit any time on additional jobs from your FitFoundry inbox.
```
...
- LaunchKit detects already-processed jobs by folder presence in results/shortlisted/.
  If you want to regenerate assets for a job, delete or rename its application folder first.
- Job reports move from results/inbox/ to results/shortlisted/ when LaunchKit runs.
  To move a job from shortlisted to applied/, update its Status: field to `applied`
  and run FitFoundry Cleanup.
